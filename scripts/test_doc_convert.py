...
def convert_docx_with_pandoc(src: Path) -> Path:
    ...
    try:
        ...
        out = mod.docx_to_md(str(src))
        return Path(out)


def convert_doc_with_doc_convert(exe_dir: Path, src: Path, dst_dir: Path) -> Path:
    # find executable
...
def main():
    ...
    if not src_doc.exists():
        ...
        out_md2 = convert_doc_with_doc_convert(exe_dir, src_doc, dst_dir)
